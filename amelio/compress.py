# -*- coding: UTF-8 -*-
# Copyright (c) 2012 Sonal Raj. This program is open-source software,
...
class CompressCommandBase(Command):
    # Options tuples: long name, short name and help string
    user_options = [
        ('sources=', None, 'sources files'),
        ('output=', None, 'compressed output filename. If you provide a template'
                          ' output filename (e.g. "static/%s-min.ext"), the'
                          ' source files will be compressed/minified individually'),
    ...
    def _create_temporary_file():
        handle, path = tempfile.mkstemp(prefix='amelio')
        os.close(handle)
        return path
    ...
    def _compress_operation(self, sources, output):
    ...
    def run(self):
        ...
        if '%' in self.output:
            ...
            for source, output in sources_outputs.items():
                ...
                self.execute(self._compress_operation,
                             ([source], output),
                             msg=msg)
        else:
...
class compress_css(CompressCommandBase):
    description = """compress CSS resources"""
    minification_type = 'css'


class compress_js(CompressCommandBase):
    description = """compress JS resources"""
    minification_type = 'js'

    user_options = CompressCommandBase.user_options + [
        ('nomunge', None, 'Compress only, do not obfuscate'),
    ...
    boolean_options = CompressCommandBase.boolean_options + _js_options
    compressor_options = CompressCommandBase.compressor_options + _js_options

    def initialize_options(self):
        CompressCommandBase.initialize_options(self)
        self.nomunge = 0
